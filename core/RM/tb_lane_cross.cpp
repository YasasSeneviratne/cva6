...
int main(int argc, char** argv, char** env) {
	...
	//Simulation of the vrilog model
	dut->pc_i_tb 		+= 1;
	...
	//Simulation of the cpp model
	//It is guaranteed that the RTL design will have alteals one cycle after reset for a valid vector to come in
        //Therefore, cpp_model must skip that cycle
        for(int i=0; i<NUM_LANES; i++){
            sim_out[i] = 0;
	    ...
	    if (sim_out[i] != monitor_o[i])
	    	cout<<"Lane: "<<i<<" input vector: "<<lane_vector[i] <<"model output: "<< sim_out[i] <<"sim output: "<< monitor_o[i] << endl; 	
